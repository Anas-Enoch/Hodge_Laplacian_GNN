...
## Corrected Multilayer Results (Rebuilt Schema)

Legacy TNBC outputs were converted into the modern validation schema to enable
corrected energy-matched Zeta and KTS analyses. Outputs are in `Results_TNBC_rebuild_gse278936/`.

**Region label mapping (legacy → modern):**
...
**Corrected multilayer cohort summary:**

| Layer | Diagnostic | Result | Interpretation |
|---|---|---|---|
| **Local** | Coexact enrichment (Step 7) | 40/40; sign test p < 10⁻¹² | Interface concentrates non-gradient interaction intensity |
...
| **Geometric curl** | Graph-curl proxy | Not used as cohort phenotype | Reconstructed TNBC geometry does not support a corrected graph-curl cohort claim |

**Corrected rebuild commands:**
...
**Legacy diagnostics status:**
- Step 20 NCG and Step 21 Zeta are retained as operator-grounding diagnostics. They should not be cited as primary evidence of interface spectral organization under the corrected framework.
- Graph-curl proxy outputs from the rebuild are not reported as a cohort-level phenotype.
...
Results_TNBC_rebuild_gse278936/cohort_zeta_energy_matched_null.csv
Results_TNBC_rebuild_gse278936/kts_transition_bias_summary.csv
